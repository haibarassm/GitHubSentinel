...
from arxiv_client import ArxivClient  # 导入arXiv客户端
...
def generate_arxiv_report(model_type, model_name, days):
    """生成arXiv论文报告"""
    config.llm_model_type = model_type
    ...
    # 获取arXiv论文数据
    filepath, papers = arxiv_client.export_papers_by_days(days=days)

    if not filepath or not papers:
        return "❌ 没有获取到符合条件的论文，请尝试增加天数或更换分类", None

    # 生成报告
    report, report_file_path = report_generator.generate_arxiv_report(filepath)
...
def update_model_list(model_type):
    if model_type == "openai":
        return gr.Dropdown(choices=["deepseek-chat", "deepseek-coder"], label="选择模型")
    elif model_type == "ollama":
        return gr.Dropdown(choices=["deepseek-r1:8b", "llama3.1:8b"], label="选择模型")
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("GitHub 项目进展"):
        ...
        # 创建 Radio 组件
        model_type = gr.Radio(["openai", "ollama"], label="模型类型", info="使用 DeepSeek API 或 Ollama 私有化模型服务")

        # 创建 Dropdown 组件
        model_name = gr.Dropdown(choices=["deepseek-chat", "deepseek-coder"], label="选择模型")
    ...
    # 创建 arXiv 论文 Tab
    with gr.Tab("arXiv 论文趋势"):
        gr.Markdown("## arXiv 论文技术趋势")  # 添加小标题
        ...
        # 创建 Slider 组件
        days = gr.Slider(value=1, minimum=1, maximum=7, step=1, label="时间范围", info="获取近N天的论文（1-7天）")

        # 使用 radio 组件的值来更新 dropdown 组件的选项
        model_type.change(fn=update_model_list, inputs=model_type, outputs=model_name)

        # 创建按钮来生成报告
        button = gr.Button("生成论文趋势报告")
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_arxiv_report, inputs=[model_type, model_name, days], outputs=[markdown_output, file_output])
